...
'재무선배'가 공개한 펀더멘털 선별 기준을 자동화한 종목 스크리너입니다.
...
`engine.py` 한 곳에 정의된 단일 기준 — 세 시장 공통:

| 단계 | 기준 |
|---|---|
| ① 유동성 | 최근 10거래일 평균 거래대금 ≥ 100억원 / $10M |
| ② 재무건전 | 부채비율 < 200% · 매출성장(YoY) ≥ 10% · TTM 영업현금 > 0 · 자본총계 > 0 |
| ③ 품질성장 | TTM 영업이익률 ≥ 10% · 이익피크(최근 4분기 영업이익=과거 최고) · 원가율 개선 |
...
- 키·승인 불필요(DART API 키는 수집기 측). 비율 기준이라 환율 무관(거래대금만 $10M ≈ 100억 등가).
