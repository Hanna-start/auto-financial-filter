...
## 2. 현재 상태 (주가 2026-06-19 기준) — 세 시장 완성·실행 완료
...
|---|---|---|---|---|---|
| 코스피 | 835 | 203 | 48 | **15** | dashboards/dashboard_kospi.html |
| 코스닥 | 1,816 | 169 | 58 | **17** | dashboards/dashboard_kosdaq.html |
| 미국 S&P500 | 499 | 499 | 107 | **52** | dashboards/dashboard_us.html |
| 미국 전체(거래소 상장) | 2,343 | 1,692 | 346 | **113** | dashboards/dashboard_us_all.html |
...
- ✅ 전체 코드 리뷰 + 외과적 수정 5종(2026-06-19): 멀티에이전트 리뷰(4관점→적대검증, 17건 중 14 확정) 후 적용. ①**연속성 가드**(`engine.compute`): `qk[-4:]`가 인접 4분기가 아니면 `None`(상류 결측으로 분기 빠진 회사의 '비연속 TTM' 오염 차단 — KR 최종 2건 제외). ②**KR 최소거래일 가드**(`fetch_krx_marcap`): 유효 거래일<10이면 amount=None(미국과 대칭, 단발 거래일 거짓통과 차단 — 코스닥 ①유동성 200→169). ③**음수분모 가드**(rev_yoy·op_margin): truthy→`is not None and >0`(음수 분모 부호뒤집힘 차단). ④문서 7곳 정정('10거래일'→20 등). ⑤README '경로 전제' 단락. 결과: 코스피17→15·코스닥19→17·미국 불변. 상세·기각오탐은 메모리 [[project-kospi-quarterly]].
