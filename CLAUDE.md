...
## 2. 현재 상태 (2026-06-08) — 세 시장 완성·실행 완료

| 시장 | 대상 | 유동성 | 재무건전 | **최종** | 산출 |
|---|---|---|---|---|---|
| 코스피 | 835 | 180 | 46 | **15** | dashboards/dashboard_kospi.html |
| 코스닥(상위300) | 300 | 116 | 39 | **13** | dashboards/dashboard_kosdaq.html |
| 미국 S&P500 | ~500 | ~500 | ~103 | **27** | dashboards/dashboard_us.html |
| 미국 전체(거래소 상장) | 2,343 | 2,297 | ~432 | **71** | dashboards/dashboard_us_all.html |
...
## 4. 필터 기준 — engine.py 한 곳

**기준을 바꾸려면 [engine.py]만 수정**(상수 + `screen()`). 세 시장 일괄 반영.
거래대금 기준만 시장별 통화라 run 스크립트에(KRW=`MIN_TRADING_KRW`, USD=`MIN_TRADING_USD`) 두고 `screen()`에 인자로 넘긴다.

7기준(원래 재무선배 엄격) + `자본총계>0`:
1. 거래대금 ≥ 100억원/$10M  2. 부채비율 < 200%  3. 매출성장 YoY ≥ 10%  4. TTM 영업현금 > 0
5. TTM 영업이익률 ≥ 10%  6. 이익피크(최근 4분기 영업이익=과거 최고)  7. 원가율 개선  (+ 자본총계 > 0)

판단 원칙: 규모·수익성=TTM(4분기 합), 성장=YoY, 분기값=누적차감 환산. 미국은 `RECENT_QUARTERS=20`으로 최근 5년만(EDGAR 장기이력에서 '피크' 비교창을 한국과 동일하게).
...
- 코스닥 잔여 수집(상위 300 외 ~1,516개) — DART 누적 `--limit` 배치.
