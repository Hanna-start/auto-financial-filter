...
<div class="p"><h2>단계별 깔때기</h2>{fhtml}<p class="lg">판단: 규모·수익성=TTM(최근 4분기 합), 성장=YoY(같은 분기 전년). 분기값은 누적 차감으로 환산. 기준(미국): 거래대금≥${MIN_TRADING_USD/1e6:.0f}M·부채비율&lt;{MAX_DEBT:.0f}%·매출성장≥{MIN_GROWTH_YOY:.0f}%·영업이익률(TTM)≥{MIN_OP_MARGIN:.0f}%·이익피크({US_CRITERIA.min_peak_quarters}분기↑)·TTM영업현금&gt;0·자본&gt;0 or(자본&lt;0&amp;이익잉여금&gt;0) · (원가율개선 미적용)</p></div>
