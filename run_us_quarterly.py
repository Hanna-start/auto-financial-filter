...
# 검증된 재무 엔진 재사용 (한국 스크리너에서 그대로)
from run_kospi_quarterly import (load_company, compute, add_valuation,
                                 MIN_OP_MARGIN, MAX_DEBT, MIN_GROWTH_YOY)
...
# 거래대금 기준: 100억원 ≈ $7M. $10M/일 평균 달러거래대금으로 설정.
# (S&P500은 이미 유동성 선별된 대형주라 대부분 통과 — 실질 선별은 재무·품질 단계)
MIN_TRADING_USD = 10_000_000

# 최근 창(분기). EDGAR는 17년+ 전 이력을 주지만, 한국은 2022~2026(~17분기)만 수집했다.
# '이익피크·원가율개선'은 최근 추세를 보는 기준이므로, 한국과 동일 의미가 되도록
# 미국도 최근 N분기로 잘라 비교(2008년 대비 같은 무의미 비교 방지). = 충실한 이식.
RECENT_QUARTERS = 20
...
def screen_us(m, trading_usd):
    """한국 screen()과 동일 로직, 거래대금만 달러 기준."""
    liq = trading_usd is not None and trading_usd >= MIN_TRADING_USD
...
def main(index_paths=None, label="S&P500", title="미국(S&P500) 분기 스크리너 결과",
         dash="dashboard_us.html", xlsx_prefix="미국_분기_결과"):
